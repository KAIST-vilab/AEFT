...
class Visualizer():
    ...
    def plot_mask_gpp(self, imgs, imgs_remain, masks, mask_remain, gpp, labels, all_preds, remain_preds, reset=True):
        if reset:
            self.vis.close(env="mask")
        # self.vis.text(labels, env="mask")
        self.vis.images(imgs, opts=dict(title='image', caption=all_preds), env="mask")
        self.vis.images(masks, opts=dict(title='cam', caption=labels), env="mask")
        self.vis.images(gpp, opts=dict(title='gpp', caption=labels), env="mask")
        self.vis.images(imgs_remain, opts=dict(title='image remain', caption=remain_preds), env="mask")
        self.vis.images(mask_remain, opts=dict(title='cam remain', caption=labels), env="mask")
