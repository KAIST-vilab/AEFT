...
def infer_aff(args, model, infer_data_loader, rw_path):
    ...
    for iter, (name, img) in enumerate(infer_data_loader):
        ...
        for k, v in cam.items():
            cam_full_arr[k+1] = v
        cam_full_arr[0] = (1 - np.max(cam_full_arr[1:], (0), keepdims=False))**19
        #cam_full_arr[0] = 0.2
...
def evaluation(rw_path):
    df = pd.read_csv('./data/VOC2012/ImageSets/Segmentation/val.txt', names=['filename'])
...
if __name__ == '__main__':
    ...
    rw_path = osp.join('./experiments', args.name, 'rw', str(args.low).zfill(2)+'_'+str(args.high).zfill(2)) 
    ...
    for ep in range(args.max_epoches):
        ...
        for iter, pack in enumerate(train_data_loader):
            ...
            if (optimizer.global_step) % 100 ==0:
                ...
                ckpt_str = osp.join(ckpt_aff_path, 'aff'+str(args.low).zfill(2)+str(args.high).zfill(2)+'_'+str(ep).zfill(2)+'.pth')
                torch.save(model.module.state_dict(), ckpt_str)
                infer_aff(args, model, infer_data_loader, rw_path)
                miou = evaluation(rw_path)

                if miou>max_miou:
                    max_miou = miou
                    torch.save(model.module.state_dict(), osp.join(ckpt_aff_path, 'aff'+str(args.low).zfill(2)+str(args.high).zfill(2)+'_'+str(ep).zfill(2)+'_best.pth'))
